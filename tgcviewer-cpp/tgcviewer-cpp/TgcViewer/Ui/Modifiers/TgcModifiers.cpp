...
#include "TgcViewer/Ui/Modifiers/TgcButtonModifier.h" //required by forward declaration
...
void TgcModifiers::addButton(const string id, const string label)
{
	add(new TgcButtonModifier(id, label));
}
...
{
	TgcGui::Message message = GuiController::Instance->gui->update();
	for(unsigned int i = 0; i < this->modifiers.size(); i++)
	{
		modifiers[i]->update(message);
	}
}
