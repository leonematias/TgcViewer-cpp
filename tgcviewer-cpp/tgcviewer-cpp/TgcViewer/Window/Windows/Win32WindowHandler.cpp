...
#include "TgcViewer/Window/Windows/Win32WindowHandler.h"
#include "TgcViewer/GuiController.h" //required by forward declaration
using namespace TgcViewer;


Win32WindowHandler::Win32WindowHandler()
{
}

Win32WindowHandler::Win32WindowHandler(const Win32WindowHandler& other)
{
}
...
void Win32WindowHandler::initializeWindow(GuiControllerConfig config)
{
	this->applicationName = config.title.c_str();
...
{
	Win32WindowHandler* windowHandler = (Win32WindowHandler*)GuiController::Instance->windowHandler;
	...
	switch(umessage)
	{
		...
		case WM_KEYDOWN:
		{
			...
			GuiController::Instance->input->setKeyDown(windowHandler->KEYBOARD_MAPPINGS[(unsigned int)wparam]);
			return 0;
		}
		//Keyup
		case WM_KEYUP:
		{
			...
			GuiController::Instance->input->setKeyUp(windowHandler->KEYBOARD_MAPPINGS[(unsigned int)wparam]);
			return 0;
		}
...
void Win32WindowHandler::stop()
{
	this->running = false;
}

void Win32WindowHandler::closeWindow()
{
...
string Win32WindowHandler::getCurrentDir()
{
	char buffer[4096];
...
void Win32WindowHandler::resetTimer()
{
	this->fps = 0;
	this->frameTime = 0;
	this->frameSecond = 0;
...
void Win32WindowHandler::updateTimer()
{
	//Compute frame time
...
long Win32WindowHandler::frequency()
{
	LARGE_INTEGER freq;
	freq.QuadPart = 0;
...
void Win32WindowHandler::showErrorMessageBox(string title, string text)
{
	Win32WindowHandler* windowHandler = (Win32WindowHandler*)GuiController::Instance->windowHandler;
	MessageBox(windowHandler->hwnd, text.c_str(), "title", MB_OK);
}

void Win32WindowHandler::initKeyboardMapping()
{
